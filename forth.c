...
void interp(cell_t *ip, cell_t *dsp, cell_t *rsp, cell_t *user, cell_t *heap,
    void (*emit_func)(cell_t), void (*terminate_func)(void)) {
  cell_t tos = *dsp--;
  cell_t *w, *x;
  ...
  DEFC("cell_t", cell_tw, eightdiv, sizeof(cell_t));
  DEFS("cell_t+", cell_tadd, cell_tw) { tos += sizeof(cell_t); NEXT; }
  DEFS("cell_t-", cell_tsub, cell_tadd) { tos -= sizeof(cell_t); NEXT; }
  DEFS("cell_ts", cell_ts, cell_tsub) { tos *= sizeof(cell_t); NEXT; }

  DEFS("+", add, cell_ts) { tos += *dsp--; NEXT; }
  ...
  DEF(emit, nzbranch) { emit_func(tos); DROP; }
  DEF(terminate, emit) { terminate_func(); }
  ...
  DEFWX(";", semicolon, F_IMMEDIATE, colon)
    W(_dict_head), W(load), L(D_FLAGS - D_NEXT), W(cell_ts), W(add), W(dup),
