...
static void interp(cell *ip, cell *dsp, cell *rsp, cell *user, cell *heap) {
  cell tos = *dsp--;
  cell *w, *x;

  goto start;

  enter: *++rsp = (cell) ip; ip = w + D_CODE_TO_REST; NEXT;
  ...
  DEFC("cell", cellw, eightdiv, sizeof(cell));
  DEFS("cell+", celladd, cellw) { tos += sizeof(cell); NEXT; }
  DEFS("cell-", cellsub, celladd) { tos -= sizeof(cell); NEXT; }
  DEFS("cells", cells, cellsub) { tos *= sizeof(cell); NEXT; }
  ...
  DEFS("@", load, zgreater) { tos = *(cell *) tos; NEXT; }
  DEFS("c@", cload, load) { tos = *(byte *) tos; NEXT; }
  DEFS("!", store, cload) { *(cell *) tos = *dsp--; DROP; NEXT; }
  DEFS("c!", cstore, store) { *(byte *) tos = *dsp--; DROP; NEXT; }
  DEFS("+!", add_store, cstore) { *(cell *) tos += *dsp--; DROP; NEXT; }
  ...
  DEF(swap, over) { x = (cell *) *dsp; *dsp = tos; tos = (cell) x; NEXT; }
  DEF(pick, swap) { DUP; tos = dsp[-tos]; NEXT; }

  DEFS("rp@", rpload, pick) { DUP; tos = (cell) rsp; NEXT; }
  DEFS("sp@", spload, rpload) { DUP; tos = (cell) dsp; NEXT; }
  DEFS("rp!", rpstore, spload) { rsp = (cell *) tos; DROP; NEXT; }
  DEFS("sp!", spstore, rpstore) { dsp = (cell *) tos; DROP; NEXT; }
  ...
  DEFS(",", comma, ccomma) {
    *CP(user[U_HEAP]) = tos; user[U_HEAP] += sizeof(cell); DROP; NEXT; }
  DEFS("s,", scomma, comma) {
    cmove((const char *) *dsp, (char *) user[U_HEAP], tos);
    user[U_HEAP] += tos; DROP; DROP; NEXT; }
  DEF(align, scomma) {
      user[U_HEAP] = (user[U_HEAP] + sizeof(cell) - 1) &
                     ~(sizeof(cell) - 1); NEXT; }
  ...
  DEFS("0branch", zbranch, branch) {
    if (!tos) { ip = CP(*ip + (cell) ip); } else { ++ip; } DROP; NEXT; }
  ...
  DEFW(_one_word, find)
      ...
      W(nzbranch), CP(3 * sizeof(cell)), W(execute), W(exit), W(comma), ENDW
  DEFW(quit, _one_word)   W(_one_word), W(branch), CP(-2 * sizeof(cell)), ENDW
